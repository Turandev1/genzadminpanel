...
export function LoginPage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (caught instanceof Error && caught.message === "MFA_CHALLENGE")
        setMfaStage(true);
      else
        setError(
    ...
    <Box className="login-shell">
      <Box className="login-story">
        <Box className="login-brand">
          <Box className="brand-mark brand-mark-large">
            <span>G</span>
          </Box>
          <span>GEN Z CLUB</span>
        </Box>
        <Box className="login-story-copy">
          ...
          <Typography component="h1">
            Community-ni
            <br />
            <em>asanlıqla</em> idarə et.
          </Typography>
          <Typography>
            Klublar, tədbirlər, ambassador proqramı və əməliyyatlar — bir
            təhlükəsiz səthdə.
          </Typography>
        </Box>
        <Box className="login-signal-card">
          <Box>
            ...
            <Typography>Platform statusu</Typography>
          </Box>
          <strong>99.98%</strong>
          <span>Son 30 gün uptime</span>
        </Box>
      </Box>
      <Box className="login-panel">
        <Box component="form" onSubmit={submit} className="login-form">
          ...
          <Button
            ...
            disabled={pending}
          >
            {mfaStage ? "Təsdiqlə" : "Təhlükəsiz giriş"}
          </Button>
          <Box className="security-note">
            <SecurityOutlinedIcon />
            <span>
              Access token yalnız yaddaşda saxlanılır. Sessiya HttpOnly cookie,
              exact-origin və CSRF nəzarəti ilə qorunur.
            </span>
          </Box>
          {runtimeConfig.demoMode && (
            <Box className="demo-roles">
              <Typography>Demo rol seçimi</Typography>
